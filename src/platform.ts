...
import { TibberPriceSensor } from './platformAccessory';
 ...
 */
export class TibberRadar implements DynamicPlatformPlugin {
  ...
  constructor(
    ...
    this.api.on('didFinishLaunching', () => {
      ...
      this.accessories = this.accessories.filter((accessory) => {
        if (this.config.radar.map(i => this.api.hap.uuid.generate(i.id)).includes(accessory.UUID)) {
   ...
   */
  discoverDevices() {
    const devices = this.config.radar;
    ...
    for (const device of devices) {
      ...
      if (existingAccessory) {
        ...
        // this is imported from `platformAccessory.ts`
        new TibberPriceSensor(this, accessory);
